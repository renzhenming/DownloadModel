...
public class DownloadManager {
	...
	public synchronized void download(String downloadUrl) {
		...
		String downloadId = Md5Utils.md5(downloadUrl);
		if (TextUtils.isEmpty(downloadId)){
			return;
		}
		DownloadInfo downloadInfo = cacheManager.getCache(downloadId);
		if (downloadInfo == null) {
			// 如果为空,表示第一次下载, 需要创建下载对象, 从头开始下载
			downloadInfo = new DownloadInfo(downloadUrl,downloadId);
		}else{
		...
		// 将下载对象保存在集合中
		cacheManager.setCache(downloadId, downloadInfo);
		// 初始化下载任务
		...
		// 将下载任务对象维护在集合当中
		mDownloadTaskMap.put(downloadId, downloadTask);
	}

	/**
	 * 下载暂停
	 */
	public synchronized void pause(String downloadUrl) {
		if (TextUtils.isEmpty(downloadUrl)){
			throw new NullPointerException("downloadUrl is null");
		}
		String dowloadId = Md5Utils.md5(downloadUrl);
		if (TextUtils.isEmpty(dowloadId)) {
			return;
		}
		DownloadInfo downloadInfo = cacheManager.getCache(dowloadId);
		if (downloadInfo == null) {
			throw new NullPointerException(downloadUrl+" is not in downloading");
		}
		int state = downloadInfo.currentState;
		// 如果当前状态是等待下载或者正在下载, 需要暂停当前任务
		if (state == STATE_WAITING || state == STATE_DOWNLOAD) {
			// 停止当前的下载任务
			DownloadTask downloadTask = mDownloadTaskMap.get(dowloadId);
	...
	public DownloadInfo getDownloadInfo(String downloadUrl) {
		...
		return cacheManager.getCache(downloadId);
	}
